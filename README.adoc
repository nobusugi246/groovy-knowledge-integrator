...
まず「Your Name」フィールドに名前を記入してください。
それから Tabキーまたはマウスにより、
「Chat Message」フィールドにフォーカスを移動してください。
...
続いて表示されます。同じユーザのメッセージが続く場合は、これらは表示されません。

.Identicon
image::images/index01a.png[]

「Identicon」は、ユーザを視覚的に判別するためのアイコンです。
「ユーザ名」の文字列から生成されるため、
...
. WebHookを表示する Chat Roomを作成します。
. 作成した Chat Roomに移動し、利用する WebHookを登録します。
...
ここでは localhostの GitBucketでの設定を例に説明します。
...
. Feedを表示する Chat Roomを作成します。
. 作成した Chat Roomに移動し、利用する Feedを登録します。
...
以後は、これよりも新しい Feedが追加されたときに、追加分がチャットとして表示されます。
...
====== Jenkins Pipelineビルド実行結果

Jenkinsの Pipelineビルド実行結果を Feedとして登録する場合は、
以下の「RSS全ビルド」の URLを右マウスボタンによりコピーし、同様に登録します。

.Jenkins Pipelineビルド実行結果の登録
image::images/index04i.png[]


====== GitHubの開発者の Public Activity

GitHubのレポジトリは直接 Feedとしてあつかえない。
開発者であれば、その「Public Activity」タブの Feedを登録すればよい。
