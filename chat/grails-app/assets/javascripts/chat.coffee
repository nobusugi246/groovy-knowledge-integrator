...
setMessageNumberBadgeOnDay = (day, count) ->
    ...
    if '' + count is '0'
        ...
        $("[data-day='#{day}']").html "<div style='line-height:90%;'>#{dn}<br/><div class='label label-info' style='font-size:9.5px;'><div style='display:none;'>.</div>#{count}</div><div>"
...
sendFixedMessage = () ->
    ...
    message.dmtarget = ''
    if _.startsWith words[0], '@'
        message.dmtarget = words[0].substring 1
        message.text = message.text.substring words[0].length
